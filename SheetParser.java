...
public class SheetParser implements Callable<List<? extends Inventory>>{
	private Sheet sheet;
	private SheetFormat sf;
	SheetParser(Sheet sheet,SheetFormat sf){
		this.sheet=sheet;
		this.sf=sf;
	}
	
	@Override
	public List<? extends Inventory> call() throws SheetParsingException {
	...
	//gets index of first row where data is there
	   private int getStartRow(Sheet sheet) throws SheetParsingException{
		   ...
		   while(rit.hasNext()){
			   ...
			   if(cell.getCellType()==Cell.CELL_TYPE_NUMERIC){
				   /*
				    *In my case first column is Sr.no.
				    *and Data starts where sr. no is 1 ,so this is start row index
				    */
	   ...
	   /*
	    * Since there may be redundant whitespaces at bottom of the sheet
	    * so to get the end of the row where data exist  
	    */
